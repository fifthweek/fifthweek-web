...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    wiredep: {
      app: {
        ...
        exclude: [
          ...
          'bower_components/bootstrap-sass-official/static/javascripts/*'
        ]
      },
      sass: {
    ...
    usemin: {
      ...
      js: ['<%= yeoman.dist %>/static/scripts/**/*.js'],
      options: {
        assetsDirs: ['<%= yeoman.dist %>','<%= yeoman.dist %>/static/images'],
        patterns: {
          // https://github.com/yeoman/grunt-usemin/issues/235#issuecomment-33316221
          js: [
            [/(static\/images\/.*?\.(?:gif|jpeg|jpg|png|webp|svg))/gm, 'Update the JS to reference our revved images']
          ]
        }
      }
    },
    ...
    htmlmin: {
      ...
      nonViews: {
        options: {
          removeAttributeQuotes:          false // For CDN conversion grunt task compatibility
        },
        files: [{
          expand: true,
          cwd: '<%= yeoman.dist %>',
    ...
    copy: {
      nonConcatenatedFilesToDist: {
        ...
        { src: '<%= yeoman.app %>/Caching.config', dest: '<%= yeoman.dist %>/static/Web.config' },
        { src: '<%= yeoman.app %>/Caching.config', dest: '<%= yeoman.dist %>/bower_components/Web.config' }
        ]
      },
      plainCssToTmp: {
        expand: true,
    ...
    replace: {
      ...
      enableStripeLiveKey: {
        src: ['<%= yeoman.dist %>/static/scripts/scripts.*.js'],
        overwrite: true,
        replacements: [{
          from: 'window.liveStripeKey=window.testStripeKey',
          to: 'window.liveStripeKey = "pk_live_CPkFeUZtDk781oGd6R53BGsV"'
        }]
      }
    },
    ...
    'file-creator': {
      ...
      disableStripeLiveKey: {
        'app/scripts/generated/stripe-keys.js': function(fs, fd, done) {
          fs.writeSync(fd, 'window.testStripeKey=\'pk_test_ViIxeg3mtiXRrrjFYC2TJsDZ\';window.liveStripeKey=window.testStripeKey;');
          done();
        }
      },
      dist:{
  ...
  grunt.registerTask('update', 'updates source files for different scenarios', function(scenario){
    ...
    grunt.task.run([
      'file-creator:disableStripeLiveKey'
    ]);
